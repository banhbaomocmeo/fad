...
model.compile(loss='categorical_crossentropy',optimizer=RMSprop(),metrics=['accuracy', auc, f1_score])
model.summary()

tbCallBack = TrainValTensorBoard(log_dir='./bi_test/logs', histogram_freq=0, write_graph=True)
ckptCallBack =  ModelCheckpoint(filepath='./bi_test/best.h5', monitor='val_loss', verbose=1, save_best_only=True, mode='max')
reduceLRCallBack = ReduceLROnPlateau(monitor='val_loss', factor=0.3, patience=5, verbose=0, min_lr=1e-6)
